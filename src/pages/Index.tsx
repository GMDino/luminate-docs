...
interface UploadedFile { id: string; name: string; size: number; type: string; content?: string; }
...
const Index = () => {
  ...
  const handleFilesUploaded = (newFiles: UploadedFile[]) => setFiles(prev => [...prev, ...newFiles]);
  const handleFileRemove = (fileId: string) => {
    setFiles(prev => prev.filter(f => f.id !== fileId));
    setSelectedSources(prev => { const newSet = new Set(prev); newSet.delete(fileId); return newSet; });
    if (selectedFileId === fileId) setSelectedFileId(null);
  };

  const handleToggleSource = (id: string) => {
    setSelectedSources(prev => { const newSet = new Set(prev); newSet.has(id) ? newSet.delete(id) : newSet.add(id); return newSet; });
  };

  const handleSelectAllSources = () => {
    selectedSources.size === files.length ? setSelectedSources(new Set()) : setSelectedSources(new Set(files.map(f => f.id)));
  };

  const selectedFile = files.find(f => f.id === selectedFileId) || null;

  // Sidebar resizing
  const startResize = () => { isResizing.current = true; document.body.style.cursor = "col-resize"; };
  const handleMouseMove = (e: MouseEvent) => { if (!isResizing.current) return; setSidebarWidth(Math.min(Math.max((e.clientX / window.innerWidth) * 100, 20), 60)); };
  const stopResize = () => { isResizing.current = false; document.body.style.cursor = "default"; };

  useEffect(() => { window.addEventListener("mousemove", handleMouseMove); window.addEventListener("mouseup", stopResize); return () => { window.removeEventListener("mousemove", handleMouseMove); window.removeEventListener("mouseup", stopResize); }; }, []);
      ...
      <header className="sticky top-0 z-10 border-b border-border bg-card/80 backdrop-blur-md shadow-sm">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="rounded-lg p-2 bg-gradient-to-br from-primary to-accent shadow"><BookOpen className="h-6 w-6 text-primary-foreground" /></div>
            <h1 className="text-2xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">PluteQ</h1>
          </div>
          <div className="flex items-center gap-3">
            <span className="hidden sm:block text-sm text-muted-foreground">Smart Document Workspace</span>
            <button className="sm:hidden p-2 rounded hover:bg-border/20" onClick={() => setMobileSidebarOpen(true)}><Menu className="w-6 h-6" /></button>
          </div>
        </div>
      </header>
          ...
          <div className="h-full flex flex-col p-6 overflow-auto no-scrollbar">
            {files.length > 0 && (
              <div className="mb-4 flex items-center justify-between">
                <span className="text-sm text-muted-foreground">{selectedSources.size} selected</span>
            ...
            {/* FileUpload shrinks if files exist */}
            <div className="mb-4">
              <FileUpload onFilesUploaded={handleFilesUploaded} small={files.length > 0} />
            </div>
